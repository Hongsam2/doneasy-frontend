...
function Comments({comments, projectId}) {
    ...
    const onReportClick = () => {
        ...
        axios.post("http://localhost:8080/report-of-comment/save-report", report)
            .then((response) => {
                ...
                <li className="w-700 mt-0 mb-0 m-auto pt-3 pl-0 pr-0 pb-3 bg-none">
                    <div className="cmt_info relative -top-2">
                        ...
                        <span className="info_append pt-02">
                            ...
                            <button onClick={onReportClick} type="button" className="btn_report pr-0 pl-570 text-sm text-[#9f9f9f] align-[0] underline">신고</button>
                        </span>
                    </div>
                </li>
            </ul>}
